...
import { ApiDocument, ApiError, ApiResources } from "./schema";

export class RoutesApiServiceConfig {
  root:string;
}
...
export class RoutesApiService {
  ...
  resources:Promise<ApiResources>;

  constructor(private http:HttpClient, private config:RoutesApiServiceConfig) {
  ...
  loadResources(){
    this.resources = this.http.get<ApiDocument>(this.root).toPromise().then(api => api._links);
  }
